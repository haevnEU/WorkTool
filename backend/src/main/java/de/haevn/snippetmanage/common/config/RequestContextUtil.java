 ...
 * This utility class provides methods to get information about the current request.
 */
@Component
public class RequestContextUtil {
     ...
     * This method retrieves the current HTTP request and returns it as an {@link Optional}.
     *
     * @return An {@link Optional} containing the current HTTP request or an empty {@link Optional} if no request is found.
     */
     ...
     * This method retrieves the application name from the current HTTP request.
     *
    ...
    /**
     * <h2>getIpAddress()</h2>
     * This method retrieves the IP address of the current HTTP request.
     * It first tries to get the IP address from the "X-FORWARDED-FOR" header.<br>
     * If that fails, it tries to get the IP address from the remote address.<br>
    ...
    /**
     * <h2>getRemoteHost()</h2>
     * This method retrieves the remote host of the current HTTP request.
     * It first tries to get the remote host from the current HTTP request.<br>
     * If that fails, it returns "unknown".
     *
    ...
    public boolean isMobileRequest() {
        final HttpServletRequest request = getCurrentHttpRequest();
        String userAgent = request.getHeader("User-Agent");
        return userAgent != null && userAgent.matches(".*(Mobile|iPhone|Android).*");
    }
}
